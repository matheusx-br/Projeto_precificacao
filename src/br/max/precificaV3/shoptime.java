    ...
    public void Execshoptime() {
        ...
        try {
            ...
            while (sqlite.next()) {
                ...
                String url = "http://www.submarino.com.br/produto/" + AD_SKU;
                String responseD = new ObtemIP().main(url);
                int ic = responseD.indexOf("src__BestPrice-sc-1jnodg3-4 cHIPUl priceSales") + 57;
                int f = ic + 7;
                int in = responseD.indexOf("LinkUI-sc-1soz7d4-0 gkEYXw") + 28;
                int fi = in + 7;
                ...
                if(loja != null) {
                    ...
                    tgfexc.executeUpdate("UPDATE TGFEXC SET AD_PRECO_SHOP = '"+preco+"' WHERE NUTAB= (select max(NUTAB) from tgftab where CODTAB=1) AND CODPROD = "+CODPROD);
                    tgfexc.executeUpdate("UPDATE TGFEXC SET AD_LOJASHOP = '"+loja+"', AD_DATA_BUYBOX = SYSDATE WHERE NUTAB= (select max(NUTAB) from tgftab where CODTAB=1) AND CODPROD = "+CODPROD);
